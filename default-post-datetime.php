...
Copyright (c) 2012-22 by Greg Ross
...
function default_post_datetime_hook( $data , $postarr )
	...
	if( empty( $postarr['post_date'] ) || '0000-00-00 00:00:00' == $postarr['post_date'] )
		...
		if( !array_key_exists( 'usenextopenday', $options ) ) { $options['usenextopenday'] = 'off'; }
		if( !array_key_exists( 'disable', $options) || !is_array( $options['disable'] ) ) { $options['disable'] = array();	}
		...
		if( ! ( $options['date'] == '' && $options['time'] == '' && $options['uselastpost'] != 'on' && $options['usenextopenday'] != 'on' ) )
			{
			...
			if( $options['uselastpost'] == 'on' && ( $options['date'] == '' || $options['time'] == '') )
				{
			...
			// If the usenextopenday option is enabled, find the next day without a post currently scheduled.
			if( $options['usenextopenday'] == 'on' && $options['date'] == '' )
				{
				// Get all future posts.
				$result = $wpdb->get_results( "SELECT ID FROM " . $wpdb->posts . " WHERE post_status = 'future' AND post_type = '" . $data['post_type']. "' ORDER BY post_date ASC", ARRAY_N );

				// If we found some, figure out if there are any gaps.
				if( ! empty( $result ) )
					{
					// Set the starting day/month/year to check against.
					$last_posttime = $starttime;

					// Setup a placeholder to store the gap date.
					$gap_date = $starttime + 86400;

					foreach( $result as $post )
						{
						// Get the selected post time.
						$this_posttime = get_the_time('U', $post[0] );

						// Calculate the posts date without a time.
						$last_post_date = strtotime( date( 'Y-m-d', $last_posttime ) );
						$this_post_date = strtotime( date( 'Y-m-d', $this_posttime ) );

						// If we're in a different year/month since the previous post, and more than 1 day, then use the last posts date + 1 day.
						if( $this_post_date - $last_post_date > 86400 )
							{
							$gap_date = $last_posttime + 86400;

							break 1;
							}

						$last_posttime = $this_posttime;
						}

					// Take care of the case where there are posts scheduled each day all the way to the end of the query.
					if( $gap_date == $starttime + 86400 ) {	$gap_date = $last_posttime + 86400;	}

					// Finally set the date to use.
					$options['date'] = date('Y-m-d', $gap_date);
			...
			// If the date is blank, set a date to use.
			if( $options['date'] == '' )
				{
				// Use today's date if the time is blank as well, otherwise check to see if we've already past the desired time.
				if( $options['time'] == '' )
					{
					...
					{
					// If we're already past the post time, tomorrow.
					if( strtotime( $options['time'], $starttime ) > $starttime )
						{
						$options['date'] = date('Y-m-d', $starttime);
						}
					else
						{
						$options['date'] = date('Y-m-d', $starttime + 86400);
						}
					}
				}
...
function default_post_datetime_language() {
	load_plugin_textdomain('default-post-datetime');
